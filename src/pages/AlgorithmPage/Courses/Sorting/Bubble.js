...
const Bubble = () => {
    ...
    return (
        <div className="text-xl py-10 px-14 bg-primary-1 text-white">
            <div className="text-8xl text-center text-yellow-200 py-8">Bubble Sort</div>
            <p className="pb-5">Bubble sort, also referred to as comparison sort, is a simple sorting algorithm that repeatedly goes through the list, compares adjacent elements and swaps them if they are in the wrong order. This is the most simplest algorithm and inefficient at the same time. Yet, it is very much necessary to learn about it as it represents the basic foundations of sorting.</p>
            ...
            <div className="text-3xl py-5 text-yellow-100">Understand the working of Bubble sort</div>
            ...
            <div className="text-3xl py-5 text-yellow-100">Algorithm:</div> We compare adjacent elements and see if their order is wrong (i.e a[i] {'>'} a[j] for 1 {'<='} i {'<'} j {'<='} size of array; if array is to be in ascending order, and vice-versa). If yes, then swap them.
            <div className="text-3xl py-5 text-yellow-100">Explanation:</div>

            <li>Let us say, we have an array of length n. To sort this array we do the above step (swapping) for n - 1 passes.</li>
            <li>In simple terms, first, the largest element goes at its extreme right place then, second largest to the last by one place, and so on. In the ith pass, the ith largest element goes at its right place in the array by swappings.</li>
            <li>In mathematical terms, in ith pass, at least one element from (n - i + 1) elements from start will go at its right place. That element will be the ith (for 1 {'<='} i {'<='} n - 1) largest element of the array. Because in the ith pass of the array, in the jth iteration (for 1 {'<='} j {'<='} n - i), we are checking if a[j] {'>'} a[j + 1], and a[j] will always be greater than a[j + 1] when it is the largest element in range [1, n - i + 1]. Now we will swap it. This will continue until ith largest element is at the (n - i + 1)th position of the array.</li>
            ...
            <div className="pl-5">
                <li>Best case scenario: The best case scenario occurs when the array is already sorted. In this case, no swapping will happen in the first iteration (The swapped variable will be false). So, when this happens, we break from the loop after the very first iteration. Hence, time complexity in the best case scenario is O(n) because it has to traverse through all the elements once.</li>
                <li>Worst case and Average case scenario: In Bubble Sort, n-1 comparisons are done in the 1st pass, n-2 in 2nd pass, n-3 in 3rd pass and so on. So, the total number of comparisons will be:</li>
            </div>
            Sum = (n-1) + (n-2) + (n-3) + ..... + 3 + 2 + 1 <br />
            Sum = n(n-1)/2<br /><br />
            Hence, the time complexity is of the order n2 or O(n2).
            <div className="text-2xl py-5 text-yellow-100">Space Complexity of Bubble sort </div>
            ...
            <div className="pl-5">
                ...
                <li>What is the advantage of bubble sort over other sorting techniques?</li>
